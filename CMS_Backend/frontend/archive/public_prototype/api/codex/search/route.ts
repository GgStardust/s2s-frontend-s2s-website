...
export async function GET(request: NextRequest) {
  try {
    ...
    if (livingCodexIndexer.getAllEntries().length === 0) {
      await livingCodexIndexer.indexAllFiles();
    }

    let results = livingCodexIndexer.searchCodex(query, limit);
    ...
    const totalEntries = livingCodexIndexer.getAllEntries().length;
    const provenEntries = livingCodexIndexer.getAllEntries().filter(entry => entry.proofStatus === 'proven').length;
    const averageResonance = livingCodexIndexer.getAllEntries().reduce((sum, entry) => sum + Math.sqrt(entry.resonanceVector.x**2 + entry.resonanceVector.y**2 + entry.resonanceVector.z**2 + entry.resonanceVector.w**2), 0) / totalEntries || 0;

    const bySource: { [key: string]: number } = {};
    livingCodexIndexer.getAllEntries().forEach(entry => {
    ...
    return NextResponse.json({
      ...
      results: limitedResults.map(entry => ({
        ...
        source: entry.source,
        contentType: entry.metadata.sourceType,
        orbAssociations: entry.orbAssociations,
        tags: [], // CodexEntry doesn't have tags, using empty array
        resonanceVector: entry.resonanceVector,
        proofStatus: entry.proofStatus,
        proofLogId: entry.proofId,
        createdAt: entry.metadata.indexedAt,
        updatedAt: entry.metadata.indexedAt,
      })),
      statistics: {
        totalEntries,
        bySource,
