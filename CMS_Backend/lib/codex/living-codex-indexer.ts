...
import { proofLogger } from '../proofs/proof-logger';
...
class LivingCodexIndexer {
  ...
  private async indexBackboneFiles(): Promise<CodexEntry[]> {
    ...
    try {
      // Index stardust_to_sovereignty_backbone.md
      const backbonePath = join(process.cwd(), '09_PROCESSED', '02c_Supporting material', 'Stardust to Sovereignty Backbone_ORIGINAL.md');
  ...
  private async indexCodexFiles(): Promise<CodexEntry[]> {
    ...
    try {
      // Index codex_Orb_Synthesis_Final.md
      const codexPath = join(process.cwd(), '09_PROCESSED', '02c_Supporting material', 'codex_Orb_Synthesis_Final.md');
      ...
      // Index S2S — Undercurrents Codex.md
      const undercurrentsPath = join(process.cwd(), '09_PROCESSED', '02c_Supporting material', 'S2S — Undercurrents Codex.md');
  ...
  private async indexOrbEssays(): Promise<CodexEntry[]> {
    ...
    try {
      const orbEssaysPath = join(process.cwd(), '09_PROCESSED', '02d_Orb_Essays');
  ...
  private async indexS2SCodexEssays(): Promise<CodexEntry[]> {
    ...
    try {
      const s2sCodexPath = join(process.cwd(), '09_PROCESSED', '02f_S2S_codex_essays');
  ...
  private async createCodexEntry(
    ...
    try {
      ...
      return {
        ...
        sovereignLogic: {
          validity: analysis.mathematical.sovereignLogic.validity === 'proven' ? 'proven' : 
                   analysis.mathematical.sovereignLogic.validity === 'partial' ? 'inconclusive' : 'disproven',
          proofSteps: [`Coherence: ${analysis.mathematical.sovereignLogic.coherence}`, `Sovereignty: ${analysis.mathematical.sovereignLogic.sovereignty}`],
          logicalConsistency: analysis.mathematical.sovereignLogic.coherence
        },
        cocValidation: {
          coherenceScore: analysis.mathematical.coherenceMatrix.coherenceRank,
