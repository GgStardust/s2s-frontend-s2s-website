...
**AUDIENCE:** Internal S2S Developers  
**PURPOSE:** Internal S2S system architecture documentation  
**STATUS:** Single Source of Truth for S2S System  
**Last Updated:** 2025-01-XX

> **Note:** This document describes the S2S-specific system architecture. For generic RBI product documentation (external partners/customers), see `RBI-Architecture-Service/RBI_ARCHITECTURE_COMPLETE.md`
...
**Note on Generic API:** RBI-Kernel now uses generic terminology (`categoryAssociations`) in public APIs while maintaining full backward compatibility with S2S-specific terminology (`orb_associations`). S2S code continues to work unchanged using `orb_associations`.
...
**Generic API Note:** RBI-Kernel public APIs use generic terminology (`categoryAssociations`, `categoryOverlap`) while maintaining full backward compatibility with S2S terminology (`orb_associations`, `orbOverlap`). S2S code using `orb_associations` continues to work unchanged.
...
Extract Metadata (YAML frontmatter + inline tags)
    ↓
RBI Computation (uses metadata.orb_associations for S2S, or categoryAssociations for generic use)
    ↓
...
**S2S-Specific Note:** S2S uses `orb_associations` in metadata, which is fully supported. RBI-Kernel also accepts generic `categoryAssociations` for external users, but S2S code continues to use `orb_associations` without changes.
...
- `S2S_SYSTEM_ARCHITECTURE.md` (this file) - Single source of truth for S2S system
- `RBI-Architecture-Service/RBI_ARCHITECTURE_COMPLETE.md` - Generic RBI product documentation (external)
